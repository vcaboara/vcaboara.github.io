...
def check_sensitive_data(file_path):
    ...
    if re.search(patent_pattern, content):
        issues.append(f"❌ CRITICAL: Patent number [REDACTED] found (should be [REDACTED])")
    ...
    for name in names:
        if name in content and 'stacy' not in name.lower():
            ...
            if f'{name}@' in content or f'{name.lower()}@' in content:
                issues.append(f"⚠️  WARNING: Individual name '{name}' exposed in email address")
    ...
    if 'Arboreum Commercial Solutions' in content:
        if not re.search(r'Arboreum Commercial Solutions LLC', content):
            ...
            if matches:
                issues.append(f"⚠️  WARNING: 'Arboreum Commercial Solutions' missing 'LLC' in {len(matches)} place(s)")
...
def check_html_structure(file_path):
    ...
    if details_open != details_close:
        issues.append(f"❌ UNBALANCED: {details_open} <details> tags but {details_close} </details> tags")
...
def main():
    ...
    if not all_issues:
        ...
        critical = [i for i in all_issues if 'CRITICAL' in i or 'MISSING' in i or 'UNBALANCED' in i]
        if critical:
