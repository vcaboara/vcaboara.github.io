...
class KnowledgeBase:
    ...
    def load_all(self) -> Dict[str, str]:
        ...
        if not self.context_dir.exists():
            raise FileNotFoundError(f"Knowledge base not found: {self.context_dir}")
    ...
    def build_system_prompt(self, base_prompt: str, include: List[str] = None) -> str:
        ...
        for name in include:
            if name in self.contexts:
                context_sections.append(f"## Knowledge Base: {name.replace('-', ' ').title()}\n\n{self.contexts[name]}\n")
...
"""
    ...
    def append_to_context(self, note: str, filename: str = "context.md"):
        ...
        # Reload contexts
        self.contexts[filepath.stem] = (self.context_dir / filename).read_text(encoding='utf-8')
